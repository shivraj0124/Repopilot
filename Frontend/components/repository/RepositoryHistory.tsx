...
/* ── difficulty colour ── */
function diffChipClass(difficulty?: string) {
  const d = difficulty?.toLowerCase() ?? "";
  if (d.includes("beginner") || d.includes("easy")) return "dash-chip-emerald";
  if (d.includes("intermediate")) return "dash-chip-amber";
  if (d.includes("advanced")) return "dash-chip-orange";
  if (d.includes("expert")) return "dash-chip-red";
  return "dash-chip-blue";
}
...
function EmptyHistory() {
  return (
    <div className="dash-empty mt-4">
      ...
      <p className="dash-empty-sub">Enter a GitHub URL above to run your first analysis</p>
    </div>
  );
}
...
export default function RepositoryHistory({ history }: RepositoryHistoryProps) {
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      {/* List */}
      {history.map((item) => (
        <div key={item.id} className="dash-card relative p-4 sm:p-5 overflow-hidden">
          <div className="dash-history-accent" />

          <div className="flex items-start justify-between gap-4">
            {/* Left — repo info */}
            <div className="flex items-start gap-3 min-w-0">
              <div className="dash-icon-badge-blue mt-0.5">
                <GitBranch size={14} />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm font-semibold truncate" style={{ color: "var(--heading)" }}>
                  <span style={{ color: "var(--text-dim)" }}>{item.repository.owner}</span>
                  <span style={{ color: "var(--text-faint)" }} className="mx-0.5">/</span>
                  <span>{item.repository.repoName}</span>
                </h3>

                {item.summary && (
                  <p className="text-xs mt-1 leading-relaxed line-clamp-2" style={{ color: "var(--text-dim)" }}>
                    {item.summary}
                  </p>
                )}
              </div>
          ...
          {(item.techStack || item.difficulty) && (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {item.difficulty && (
                <span className={`inline-flex items-center text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md ${diffChipClass(item.difficulty)}`}>
                  {item.difficulty}
                </span>
              )}
              ...
              {item.techStack
                ...
                ?.slice(0, 6)
                ?.map((tech: string, i: number) => (
                  <span key={i} className="dash-chip" style={{ fontSize: "0.6875rem", padding: "3px 9px" }}>
                    <span className="dash-chip-dot" style={{ width: 5, height: 5 }} />
                    {tech.trim()}
                  </span>
                ))}
