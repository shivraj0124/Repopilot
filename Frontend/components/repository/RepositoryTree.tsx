...
const EXT_ICONS: Record<string, { Icon: React.ElementType; color: string }> = {
  ...
  md:   { Icon: BookOpen, color: "text-slate-400" },
  mdx:  { Icon: BookOpen, color: "text-slate-400" },
...
function getFileIcon(name: string) {
  ...
  return EXT_ICONS[ext] ?? { Icon: File, color: "" };
}
...
function TreeNodeRow({
  ...
  const { Icon, color } = isDir
    ? { Icon: open ? FolderOpen : Folder, color: "" }
  ...
  return (
    <div>
      <div
        className={`dash-tree-row ${isDir ? "font-medium" : "font-normal"}`}
      ...
      >
        ...
        {/* icon */}
        <Icon size={14} className={`flex-shrink-0 ${isDir ? "" : color}`} style={isDir ? { color: "var(--blue)" } : undefined} />
      ...
      {isDir && open && sortedChildren.length > 0 && (
        <div className="relative">
          {/* indent guide line */}
          <div className="dash-tree-line absolute top-0 bottom-0 w-px" style={{ left: `${14 + indent}px` }} />
          {sortedChildren.map((child) => (
            <TreeNodeRow key={child.path} node={child} depth={depth + 1} defaultOpen={depth < 1} />
          ))}
        </div>
      )}
    </div>
...
export default function RepositoryTree({ tree }: RepositoryTreeProps) {
  ...
  return (
    <div className="dash-card mt-4 overflow-hidden">

      {/* ── Header ── */}
      <div className="dash-tree-header flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3.5">
        <div className="flex items-center gap-2.5">
          <div className="dash-icon-badge-blue" style={{ width: 28, height: 28 }}>
            <FolderTree size={13} />
          </div>
          <span className="text-sm font-semibold" style={{ color: "var(--heading)" }}>Repository Structure</span>
          <div className="flex items-center gap-2 ml-1">
            <span className="dash-badge !rounded-md">{dirs} folders</span>
            <span className="dash-badge !rounded-md">{files} files</span>
          </div>
        </div>

        {/* Search */}
        <div className="dash-search-wrap !flex-none">
          ...
          <input
            ...
            className="dash-search-input w-full sm:w-52 !py-1.5 !pl-8 !text-xs"
          />
        </div>
      </div>

      {/* ── Tree / Search results ── */}
      <div className="dash-scroll max-h-[480px] overflow-y-auto p-2">
        {search.trim() ? (
          filteredPaths.length > 0 ? (
            <div className="space-y-px">
              {filteredPaths.map((p: string, i: number) => {
                ...
                return (
                  <div key={i} className="dash-tree-row">
                    <Icon size={13} className={`flex-shrink-0 ${color}`} />
                    <span className="text-xs font-mono truncate">
                      {before}
                      <span className="dash-highlight">{match}</span>
                      {after}
                    </span>
                    {dotColor && (
                      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ml-auto ${dotColor} opacity-60`} />
              ...
              {tree.length > 80 && (
                <p className="text-center text-[11px] pt-2 pb-1" style={{ color: "var(--text-faint)" }}>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <File size={24} style={{ color: "var(--text-faint)", marginBottom: 8 }} />
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>No files match "{search}"</p>
            </div>
          )
        ) : (
      ...
      {/* ── Footer ── */}
      <div className="dash-tree-footer px-4 py-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {Object.keys(EXT_DOT).slice(0, 5).map((ext) => (
            <span key={ext} className="flex items-center gap-1 text-[10px]" style={{ color: "var(--text-faint)" }}>
